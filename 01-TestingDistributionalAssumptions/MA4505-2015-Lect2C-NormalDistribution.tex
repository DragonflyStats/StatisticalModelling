...
\frame{
	...
	\begin{itemize}
		...
		\item They are symmetric with observations more concentrated in the middle than in the tails.
...
\frame{
	...
	\begin{center}
		\includegraphics[scale=0.50]{images/5ABellCurve}
	\end{center}
...
\frame{
	...
	\begin{itemize}
		...
		The normal probability curve is bell-shaped and \textbf{symmetric}, with the shape of the curve to the left of the mean a mirror image of the shape of the curve to the right of the mean.
		...
		\item[5] (For Later : The Skewness is 0, the Kurtosis is 3)
	\end{itemize}
}
%------------------------------------------------------------------%
\frame{
...
\frame{
	...
	All normally distributed random variables have corresponding $Z$ values, called \textbf{Z-scores}.\\
	\bigskip
...
\frame{
	...
	\Large
	\begin{itemize}
		\item Suppose that mean $\mu = 80 $ and that standard deviation $\sigma = 8$. \bigskip
...
\frame{
	...
	\large
	\begin{itemize}
\item \textbf{Important:} A Z-score always reflects the number of standard deviations above or below the mean a particular score is.
\item Suppose the scores of a test are normally distributed with a mean of 50 and a standard deviation of 9
...
\frame{
	...
	\large
\noindent	\textbf{Murdoch Barnes Tables}
	\begin{itemize}
		...
		\bigskip
		\item Given a value of $k$ (with k usually between 0 and 4), the probability of a standard normal "Z" random variable being greater than (or equal to) k $P(Z \geq k)$ is given in \textbf{Murdoch Barnes table 3} .
		\bigskip
...
\frame{
	...
	\large
	\begin{itemize}
		\item For some value $z_o$, between 0 and 4, the Murdoch Barnes tables set 3 tabulate $P(Z \geq z_o)$ \bigskip
		\item Ideally $z_o$ would be specified to 2 decimal places. If it is not, round to the closest value.
		\bigskip
		\item We call the third digit (i.e. the digit in the second decimal place) the ``second precision". (This is a very informal term).
	\end{itemize}
}
...
\frame{
	\frametitle{Using Murdoch Barnes tables 3}
	\large	\begin{itemize}
		\item Find $ P(Z \geq 0.60)$ \bigskip
		\item Find $ P(Z \geq 1.64)$ \bigskip
		\item Find $ P(Z \geq 1.65)$ \bigskip
		\item Estimate $P( Z \geq 1.645)$ \bigskip
	\end{itemize}
}
...
\frame{
	...
	\large 
	\begin{itemize}
		\item $ P(Z \geq 1.64) = 0.505$ \bigskip
		\item $ P(Z \geq 1.65) = 0.495$ \bigskip
		\item $ P(Z \geq 1.645)$ is approximately the average value of $ P(Z \geq 1.64)$ and $ P(Z \geq 1.65)$. \bigskip
